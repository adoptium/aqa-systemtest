...
The automation depends on the [System Test Framework (STF) project](https://github.com/adoptium/stf). This
...
```
The runtime.jti file in the aqa-systemtest repository is a JCK 'interview' configuration file
...
1. Clone and build the stf (test framework) and aqa-systemtest repositories as described [here](../../README.md).
2. Set JAVA_HOME to the jdk that needs to be tested.
3. Configure the JCK as described in [Setup](#setup).
4. cd aqa-systemtest/build
5. make test.jck8b
...
```
The JCK tests also require certain configuration files.  Some of these are included in the aqa-systemtest
...
There are templates for these files included in the aqa-systemtest repository
```
...
in the aqa-systemtest repository as a template, and then substituting certain values as determined from interrogating the
